...
public class AbstractLogFilterTest {
	private AbstractLogFilter<TestMessage> logFilter;
	...
	public void setup() {
		...
		logFilter = new AbstractLogFilter<TestMessage>() {
	...
	public void testFilteringNullEntry() {
		...
		TestMessage tm = logFilter.applyFilters(null, filters);
	...
	public void testFilteringNonMatchingEntry() {
		...
		TestMessage tm = logFilter.applyFilters("A Log Entry", filters);
		verify(messageFilter, times(1)).parse(anyString());
		assertThat(tm, is(nullValue()));
	}
	...
	public void testFilteringMatchingEntry() {
		...
		TestMessage tm2 = logFilter.applyFilters("A Log Entry", filters);
		verify(messageFilter, times(1)).parse(anyString());
